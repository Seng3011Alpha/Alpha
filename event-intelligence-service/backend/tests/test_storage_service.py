...
from app.services.storage_service import save_raw, save_standardised, load_standardised
...
class TestSaveRaw:
    def test_saves_file_and_returns_path(self, tmp_path):
        with patch("app.services.storage_service.RAW_DIR", tmp_path / "raw"), \
             patch("app.services.storage_service.STANDARDISED_DIR", tmp_path / "standardised"):
...
class TestSaveStandardised:
    def test_saves_file_and_returns_path(self, tmp_path):
        with patch("app.services.storage_service.RAW_DIR", tmp_path / "raw"), \
             patch("app.services.storage_service.STANDARDISED_DIR", tmp_path / "standardised"):
            result = save_standardised(SAMPLE_DATASET, "combined_events.json")
            saved = Path(result)
            assert saved.exists()
    ...
    def test_overwrites_existing_file(self, tmp_path):
        ...
        with patch("app.services.storage_service.RAW_DIR", tmp_path / "raw"), \
             patch("app.services.storage_service.STANDARDISED_DIR", std_dir):
            save_standardised({"version": 1}, "combined_events.json")
            save_standardised({"version": 2}, "combined_events.json")
...
class TestLoadStandardised:
    def test_returns_none_when_file_missing(self, tmp_path):
        with patch("app.services.storage_service.STANDARDISED_DIR", tmp_path / "standardised"):
            result = load_standardised("does_not_exist.json")
    ...
    def test_loads_existing_file(self, tmp_path):
        ...
        with patch("app.services.storage_service.STANDARDISED_DIR", std_dir):
            result = load_standardised("test.json")
